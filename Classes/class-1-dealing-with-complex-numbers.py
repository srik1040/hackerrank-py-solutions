...
class Complex:
    ...
    def __str__(self):
        real_part = '{0:.2f}'.format(0 if self.real == 0 and self.imaginary != 0 else self.real)
        sign = '' if real_part == '' else '+' if self.imaginary >= 0 else '-' if self.imaginary < 0 else ''
        imaginary_part = '{0:.2f}i'.format(self.imaginary if real_part == '' else abs(self.imaginary) if self.imaginary != 0 else 0)
